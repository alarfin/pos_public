...
@section('photocopy_machine_add', 'active')

@section('title')
    Photocopy Machine Add
@endsection
@section('style')
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box">
                ...
                <!-- /.box-header -->
                <!-- form start -->
                <form class="form-horizontal" method="POST" action="{{ route('photocopy_machine_add') }}">
                    @csrf
                                ...
                                <select name="company_branch_id" id="company_branch_id" class="form-control select2"
                                    ...
                                    @foreach ($branches as $branch)
                                        <option value="{{ $branch->id }}"
                                            @if (old('company_branch_id') == $branch->id) selected @endif>
                                            {{ $branch->name }}
                                        </option>
                                    @endforeach
                                </select>
