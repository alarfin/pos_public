...
@section('bank_opening_balance_add', 'active')
                                    ...
                                    @if (old('account_ids') != null && sizeof(old('account_ids')) > 0)
                                        @foreach (old('account_ids') as $item)
                                            <tr class="account-item">
                                                <td>
                                                    <div
                                                        class="form-group {{ $errors->has('bank_ids.' . $loop->index) ? 'has-error' : '' }}">
                                                        <select name="bank_ids[]" class="form-control select2 bank_ids">
                                                            <option value=""> Select bank </option>
                                                            @foreach ($banks as $bank)
                                                                <option value="{{ $bank->id }}"
                                                                    @if (old('bank_ids.' . $loop->index) == $bank->id) selected @endif>
            ...
            <td>
                <div class="form-group">
                    <select name="bank_ids[]" class="form-control select2 bank_ids">
                        <option value=""> Select bank </option>
                        @foreach ($banks as $bank)
                            <option value="{{ $bank->id }}"> {{ $bank->name }}- {{ $bank->account_no }}
                            </option>
                        @endforeach
                    </select>
                </div>
