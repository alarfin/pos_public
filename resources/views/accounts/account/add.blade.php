    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box">
                ...
                <!-- /.box-header -->
                <!-- form start -->
                <form class="form-horizontal" method="POST" action="{{ route('account_add') }}">
                    @csrf
                                    ...
                                    @foreach ($account_classes as $account_class)
                                        <optgroup label="{{ $account_class->name }}">
                                            @foreach ($account_class->accountHeads??[] as $account_head)
                                                <option value="{{ $account_head->id }}" @if (old('account_id')==$account_head->id) selected @endif> - {{ $account_head->name }} </option>
                                            @endforeach
                                        </optgroup>
                                    @endforeach
                                </select>
